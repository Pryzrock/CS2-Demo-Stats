...
Десктопное приложение с тёмным GUI для анализа демок CS2 (.dem). Парсит демку и показывает скорборд с детальной статистикой по каждому игроку: K/D/A, ADR, KAST%, Rating. Результат можно сохранить в CSV.
...
pyinstaller --onedir --windowed --name "CS2DemoStats" main.py
```

Готовый exe появится в `dist/CS2DemoStats/CS2DemoStats.exe`.
